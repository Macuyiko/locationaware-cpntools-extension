...
public abstract class InfoToolHelper implements MapBoundsListener {
	...
	/**
	 * CAlled by the helper lookup system when selecting a helper for a given
	 * layer.
	 * 
	 * @param layer
	 *            the layer
	 ...
	 *             if {@code layer} is {@code null}
	 */
	public abstract boolean isSupportedLayer(Layer layer);

	/**
	 * Gets layer data at the specified position. If there are no feature data
	 * at the position, an empty {@code InfoToolResult} object is returned.
	 * 
	 * @param pos
	 *            query position
	 * @return layer data
	 ...
	 * a {@code Layer}.Helpers only hold a {@code WeakReference} to both the map
	 * content and layer to avoid blocking garbage collection when layers are
	 * discarded.
	 * 
	...
	/**
	 * A method from the {@code MapBoundsListener} interface used to listen for
	 * a change to the map content's coordinate reference system.
	 */
	@Override
	 ...
	 * {@code Layer}.
	 * 
	 * @return the transform or {@code null} if the layer's CRS is the same as
	 *         that of the map content, or if either has no CRS defined
	 */
	protected MathTransform getContentToLayerTransform() {
		if (transform == null && !transformFailed) {
			...
			if (content != null && layer != null) {
				CoordinateReferenceSystem contentCRS = content
				...
				if (contentCRS != null && layerCRS != null) {
					if (CRS.equalsIgnoreMetadata(contentCRS, layerCRS)) {
						...
						try {
							transform = CRS.findMathTransform(contentCRS,
									layerCRS, true);
						} catch (Exception ex) {
